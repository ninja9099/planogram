...
export const validateChangePosition = (graph: dia.Graph) => (err: Error, command: any, next: Function): boolean => {
...
export const validateChangeSize = (graph: dia.Graph) => (err: Error, command: any, next: Function): boolean => {
    const cell = graph.getCell(command.data.id) as dia.Element;
